...
| Результат на TPU |
|:---:|
| ![TPU — 31.9 с](docs/result_tpu.png) |
...
- **batch-CFG B=2** компилируется (3328/3328), но рантайм edgetpu падает на фатальном
  `CHECK failed: pending_sink_fds.erase(fd)` — баг закрытого рантайма на графе с двумя выходами.
