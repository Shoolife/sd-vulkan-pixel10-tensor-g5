...
Вторым бэкендом в приложение добавлен **Tensor TPU** (UNet на NPU через LiteRT, 25.4 с) — он нужен
...
| Результат на TPU |
|:---:|
| <img src="docs/result_tpu.png" width="320" alt="TPU — 25.4 с"> |
| **⏱ 25.4 с** (forward 3.9 с × 6) |
...
| Время forward | **3.88 с** (компилятор SDK v1.1.0; на v1.0.0 было 4.95 с) |

### Сравнение бэкендов на одном устройстве (один forward UNet)

| Бэкенд | Время | Примечание |
|---|---|---|
| **TPU (bf16)** | **3.88 с** | весь граф на NPU |
...
- **batch-CFG B=2 недоступен — и это чужой баг, а не предел железа.** Считать cond и uncond одним
  ...
  теперь не двукратную экономию, а 6 → 4 запуска. Перепроверено в августе 2026 на **всех**
  комбинациях — компилятор v1.0.0 и v1.1.0 × рантайм LiteRT 2.1.x и 2.2.0 × прошивка июльская
  и августовская (Android 17): падает везде одинаково, то есть баг в системном слое EdgeTPU.

### int8: проверено и отклонено

Квантизация выглядела единственным способом ускорить UNet. Проверены шесть вариантов
(`tools/quantize_int8.py`, `tools/quantize_wo8.py`), все замеры — на устройстве, на компиляторе
SDK v1.0.0 (тогда bf16 давал 4.95 с):

| Вариант | forward | corr | Итог |
|---|---|---|---|
| bf16 (эталон) | 4.95 с | 0.999996 | ✅ используется (на v1.1.0 — 3.88 с) |
...
| **обновление компилятора до SDK v1.1.0** (пересборка той же модели, ничего не меняя) | forward 4.95 → 3.88 с: 31.9 → **25.4 с** ✅ |
...
### Версии SDK: что закрыто, а что доступно всем

AOT-компилятор (`litert_plugin_compiler.tar.gz`, x86) Google выдаёт по заявке — форма на странице
[Tensor SDK](https://developers.google.com/edge/tensor-sdk); в `setup.py` пакета висит
`TODO: b/475410468 — remove once publicly available without ACLs`. Версия компилятора влияет на
скорость напрямую: пересборка **той же** модели плагином v1.1.0 (июль) вместо v1.0.0 (май) дала
forward 4.95 → **3.88 с** при неизменном corr 0.999996.

А вот остальная часть стека лежит открыто, без всякой заявки:

| Компонент | Где взять |
|---|---|
| `libLiteRtDispatch_GoogleTensor.so` | [релизы LiteRT](https://github.com/google-ai-edge/LiteRT/releases), `litert_npu_runtime_libraries.zip` |
| **`libLiteRtCompilerPlugin_google_tensor.so`** — компилятор под Tensor для arm64 | там же, `litert_npu_runtime_libraries_jit.zip` |
| `libLiteRt.so` | Maven: `com/google/ai/edge/litert/litert/2.2.0/litert-2.2.0.aar` |
| Заголовки C-API | `litert_cc_sdk.zip` в тех же релизах |

Из этого следует, что **JIT-компиляция на самом телефоне работает без закрытого SDK** —
проверено раннером `tools/npu/litert_tpu_jit.c` (опции окружения `CompilerPluginLibraryDir` +
`DispatchLibraryDir` + `CompilerCacheDir`):

| Модель | JIT-компиляция на устройстве | forward на TPU | было |
|---|---|---|---|
| CLIP (235 МБ) | 13 с, с кэшем 0.3 с | **13 мс** | ~2 с на CPU |
| TAESD (5 МБ) | 5.4 с | **26 мс** | ~1 с на GPU |
| UNet SD1.5 | ✗ `RESOURCE_EXHAUSTED: … is RESHAPE layer, and overflows memory` | — | — |

То есть UNet по-прежнему требует AOT (у JIT на телефоне меньше ресурсов и он не умеет шардить
RESHAPE из GroupNorm), а CLIP и VAE можно вынести на TPU вообще без участия Google.
...
4. ✅ **TPU Tensor** — UNet целиком на NPU G5 (3328/3328 op, corr 0.999996), forward 3.88 с,
   генерация **25.4 с**. int8 проверен и отклонён — рушит картинку; выигрыш дали CFG на 2 шагах,
   TAESD и обновление компилятора до SDK v1.1.0.
5. ⏭ **Меньшая UNet** (дистилляты BK-SDM / tiny-sd) — единственный путь пробить 3.88 с/forward на TPU;
